...
import XRReferenceSpace, { XRReferenceSpaceTypes } from "./XRReferenceSpace";
...
export default class XRSession extends EventTarget {
  ...
  constructor(device, mode, id) {
    ...
    this[PRIVATE].onDeviceFrame = () => {
      ...
      if (this[PRIVATE].pendingRenderState !== null) {
        ...
        this[PRIVATE].activeRenderState = new XRRenderState(
      ...
      this[PRIVATE].device.onFrameStart(
        this[PRIVATE].id,
        this[PRIVATE].activeRenderState
      );
    ...
    this[PRIVATE].startDeviceFrameLoop = () => {
      if (this[PRIVATE].deviceFrameHandle === null) {
        this[PRIVATE].deviceFrameHandle = this[
          PRIVATE
        ].device.requestAnimationFrame(this[PRIVATE].onDeviceFrame);
      }
    };
  ...
  async requestReferenceSpace(type) {
    ...
    if (
      !this[PRIVATE].device.doesSessionSupportReferenceSpace(
        this[PRIVATE].id,
        type
      )
    ) {
    ...
    // frame of reference.
    let transform = await this[PRIVATE].device.requestFrameOfReferenceTransform(
      type
    );
  ...
  async end() {
    ...
    if (this[PRIVATE].immersive) {
      this[PRIVATE].ended = true;
      this[PRIVATE].device.removeEventListener(
        "@@webxr-polyfill/vr-present-start",
        this[PRIVATE].onPresentationStart
      );
      this[PRIVATE].device.removeEventListener(
